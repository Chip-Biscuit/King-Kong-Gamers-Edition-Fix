...
//fix enhancers king kong patch for window mode added 
...
bool GetWindowModeSetting()
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Ubisoft\\KingKong\\{2C391F94-B8B9-4832-9C57-3AFC332CC037}\\Basic video"), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
        if (RegQueryValueEx(hKey, _T("WindowedMode"), NULL, NULL, (LPBYTE)&dwValue, &dwSize) == ERROR_SUCCESS)
        {
            RegCloseKey(hKey);
            return dwValue != 0;
        }
...
void SetWindowModeSetting(bool enable)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Ubisoft\\KingKong\\{2C391F94-B8B9-4832-9C57-3AFC332CC037}\\Basic video"), 0, KEY_WRITE, &hKey) == ERROR_SUCCESS)
    {
        RegSetValueEx(hKey, _T("WindowedMode"), 0, REG_DWORD, (const BYTE*)&dwValue, sizeof(DWORD));
        RegCloseKey(hKey);
    }
}
